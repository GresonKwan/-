...
function buildSystemPrompt() {
  return [
    ...
    "回复必须限制在3到4句话，每次只聚焦一个核心问题。",
    "当学生提出观点、判断、假设或结论时，你必须先明确判断其“正确 / 基本正确但不完整 / 有错误”，再给出正确答案或修正，并解释关键理由。",
    "允许直接回答学生的问题，包括给出本次凸透镜实验相关的正确结论或规律，但回答后必须补充一个让学生继续思考、验证或记录的问题。",
...
function buildDeepSeekUserPrompt(payload) {
  ...
  if (reference) {
    ...
    promptParts.push(
      "生成要求：以“小科”的口吻回复；如果学生表达了观点、判断、假设或结论，先判定对错或是否完整，再给出正确答案和简短理由，最后提出一个继续验证或记录的问题。"
    );
  }
...
function buildLocalAgentReply(payload) {
  ...
  if (observation && activeStep) {
    if (activeStep === "4") {
      return `我是小科，你们报告的“${observation}”可以作为一条有效证据；如果光屏上能承接到清晰像，这个判断通常是实像。下一步应记录物距、像距、像的大小和正倒，这些数据能帮助你们归纳规律。${strategy.focus} 请继续补充一次物距和像距，或让同伴复述你们这样判断的理由。`;
    }

    return `我是小科，你们报告的观察“${observation}”可以用来判断当前想法是否正确，但还需要看它对应“${stepTitle}”中的哪个问题、假设或结论。若这条观察与假设一致，就能支持假设；若不一致，就说明假设需要修正。${strategy.focus} 请补充一个更具体的数据、控制条件或判断依据。`;
  }
  ...
  if (observation) {
    return `我是小科，我先根据你们报告的观察判断：${observation}。请注意，我不能自动读取左侧 NOBOOK 实验状态，只能依据你们输入的数据和现象判断对错并给出解释。${strategy.focus} 你们可以补充物距、像距、像的虚实、大小和正倒中的任意两项。`;
  }

  if (message.includes("实像") || message.includes("虚像")) {
    return `我是小科，判断实像和虚像的关键是：实像能用光屏承接，虚像不能用光屏承接。这个判断标准是正确答案，但还要结合你们的实验现象来确认。${strategy.focus} 请你们在左侧验证一次，并把光屏上是否出现清晰像告诉我。`;
  }

  return `我是小科，你可以直接把自己的答案、判断或结论发给我，我会判断它是正确、部分正确还是有错误。需要时我也会直接给出正确答案，并说明为什么。${strategy.focus} 请先告诉我你们在左侧改变了什么、观察到了什么。`;
}

...
2. 学生在左侧嵌入的 NOBOOK 实验中操作器材，观察实像、虚像、大小与倒正变化。
...
用户指定左侧直接嵌入 NOBOOK 网页，不自行开发模拟实验。2026-05-27 已核验：

| 页面 | 地址 | 用途 |
| --- | --- | --- |
| NOBOOK 精品实验列表 | [https://wl.nobook.com/console/templates/resource](https://wl.nobook.com/console/templates/resource) | 用户提供的资源目录入口 |
| 探究凸透镜成像的规律 | [https://wl.nobook.com/console/templates/resource/207_d2c4a829c23aa7471a0344a92e34cb74](https://wl.nobook.com/console/templates/resource/207_d2c4a829c23aa7471a0344a92e34cb74) | 已核验标题、实验目的及“去做实验”入口的具体资源页 |

在课堂应用中直接加载具体资源页，避免让学生先在目录中搜索实验。若 NOBOOK 为“去做实验”提供独立的直接运行 URL，应在实现联调时将 iframe 配置进一步切换到该 URL。

### 3.3 嵌入与联动决策

| 决策 | 说明 |
| --- | --- |
| 左栏内容 | 使用 iframe 直接承载 NOBOOK 目标实验页面，不编写、复制或改造实验模型 |
| 嵌入初步可行性 | 2026-05-27 对目录页及具体资源页检查响应头，未发现 `X-Frame-Options` 或 CSP `frame-ancestors` 禁止项；上线前仍需重检 |
| 第三方访问条件 | 登录、VIP/授权、Cookie 和学校网络可达性需在联调与课堂试运行中验证 |
| 实验数据联动 | NOBOOK 是跨域第三方页面；无官方通信接口前，MVP 不读取 iframe 内部物距、焦距或成像状态 |
...
- 单页双栏实验工作台。
- 左侧以 iframe 嵌入 NOBOOK 指定的凸透镜实验资源页面。
- 右侧试验者信息表单及会话内保存。
- 五个探究步骤按钮，触发已指定提示词。
...
- 未经 NOBOOK 提供的官方协议自动读取其 iframe 内实验状态。
...
| 编号 | 需求 | 优先级 | 验收标准 |
| --- | --- | --- | --- |
| EXP-01 | 左侧嵌入 NOBOOK 凸透镜资源页 | P0 | iframe 加载配置的具体资源 URL，学生可进入“探究凸透镜成像的规律”实验 |
| EXP-02 | 不自行实现模拟器 | P0 | 仓库不包含自行编写的成像绘图、物距控制或光路计算模块 |
| EXP-03 | 提供第三方加载反馈 | P0 | 加载超时或嵌入失效时显示说明及在新标签页打开 NOBOOK 的备用操作 |
| EXP-04 | 固定实验来源配置 | P0 | 资源地址集中配置，默认指向已核验具体资源页而不是资源目录 |
| EXP-05 | 验证外部使用条件 | P1 | 联调记录登录/VIP 要求、第三方 Cookie 行为和学校网络可访问性 |
...
| CHAT-03 | 结合学生观察 | P0 | Agent 根据学生主动输入的实验数据或现象回答；不声称已读取 NOBOOK iframe 状态 |
...
- Agent 对学生呈现为“小科”，定位为嵌入协作科学推理过程的 GAI 聊天机器人智能学伴，而不是给答案的判题工具。
- 使用简洁、准确、面向初中学生的中文；每次回复限制为 3～4 句话，除必要公式或学生引用外不输出长段讲解。
- 回复结构固定为：先回应学生当前问题，再给出概念或推理提示，最后提出一个可立即观察、记录、讨论或复述的问题。
- 对于学生的观察，区分“学生报告的数据”和“尚未验证的推断”；在没有学生报告或正式接口数据时，不声称已读取 NOBOOK 实验状态。
- 第 1 至 3 步的摩擦力例子仅用于示范探究方法，回复结尾必须迁移回凸透镜实验，不将摩擦力结论混写为本实验结论。
- 禁止直接给出本次凸透镜实验的最终结论、完整规律或替学生完成推理；学生索要答案时，改为提示观察维度、控制变量和证据记录方式。
...
| 知识类型 | 支持重点 |
| --- | --- |
| 概念性知识 | 用事实线索、类比或关键判断标准帮助理解概念，但要求学生用实验观察验证。 |
| 程序性知识 | 引导学生提出假设、控制变量、设计步骤和记录数据，不代写完整方案。 |
| 认识论知识 | 追问证据标准、重复性、变量控制和结论可靠性，帮助学生修正假设。 |
...
- 不让模型声称读取到了 NOBOOK 实验状态；MVP 上下文仅可标示为学生主动报告的观察。
...
| 性能 | 自有 UI 可交互目标 <= 2 秒；NOBOOK iframe 加载耗时单独监测并提供加载/失败状态 |
| 可用性 | 核心操作无需页面横向滚动；错误可恢复；聊天文本可选择复制 |
| 可访问性 | 表单有标签；按钮可键盘操作；焦点可见；文本对比度满足 WCAG AA 基本目标 |
| 安全性 | 模型密钥不下发浏览器；iframe 仅加载白名单 NOBOOK 地址；渲染 Agent 文本时转义 HTML |
...
```ts
const NOBOOK_RESOURCE_INDEX_URL =
  "https://wl.nobook.com/console/templates/resource";

const NOBOOK_LENS_RESOURCE_URL =
  "https://wl.nobook.com/console/templates/resource/207_d2c4a829c23aa7471a0344a92e34cb74";
```

- MVP iframe 使用 `NOBOOK_LENS_RESOURCE_URL`；列表入口只作为维护人员查找资源的参考。
- 主应用不注入或修改第三方页面内容，不读取跨域 DOM。
- 如 NOBOOK 后续提供官方嵌入 SDK 或消息协议，需单独评审其来源验证、授权和数据字段后才能向 Agent 提供自动状态。
- 实现阶段须验证“去做实验”后的运行页面仍可在 iframe 中使用，并遵守 NOBOOK 服务许可。
...
1. 打开工作台，左侧 iframe 加载 NOBOOK “探究凸透镜成像的规律”资源页，右侧看到试验者信息、五按钮与聊天区。
2. 不填必填项直接保存，表单明确提示，不发送无效上下文给 Agent。
3. 填写小组信息并保存，消息区显示不含多余个人信息的确认消息。
4. 在 NOBOOK 页面操作实验，并把观察输入聊天；Agent 只能引用学生已输入的观察。
...
- 已测试桌面主布局及窄屏降级布局。
- 已在目标部署环境验证 NOBOOK iframe、登录/授权及实验启动流程。
- 指定提示词内容完成教师/需求方审阅。
...
| M2 实验嵌入 | NOBOOK iframe、加载故障态、实验启动与授权验证 |
...
- 课堂最终使用具体凸透镜资源页，还是允许学生在 NOBOOK 资源列表自主选择。
- NOBOOK 的嵌入、账号/VIP 与课堂使用授权是否满足部署要求；是否提供可读取实验数据的正式接口。
